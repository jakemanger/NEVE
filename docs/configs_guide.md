...
| `belowHorizonColourRFront`, `belowHorizonColourGFront`, `belowHorizonColourBFront`, `belowHorizonColourAFront` | An override of the RGBA values of the below horizon colour for the front display. Can also override `Right`, `Left` and `Back` displays by substituting them in for `Front` in the parameter name. THIS REQUIRES THE `horizonHeightFront`, `horizonHeightRight`, `horizonHeightLeft`,  `horizonHeightBack` to be set to work. | `0`-`1` | *Ignored* |
