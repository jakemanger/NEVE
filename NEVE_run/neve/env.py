...
class Nenv:
    ...
    def __init__(self, params):
        ...
        if not os.path.exists(params):
            print(f'Could not find the configuration file at {params}')
            params = os.path.join('NEVE_run', params)
    ...
    def _get_built_file_path(self):
        ...
        if str(self.params['buildDir']).lower() not in (
            ...
            if not os.path.exists(build_dir):
                print(f'Could not find the build directory at {build_dir}')
                if not os.path.exists(os.path.join(os.getcwd(), 'builds')):
                    print(
                        'This may be because ./builds/ is not in the working directory, '
                        'checking in ./NEVE_run/builds/'
                    )

                    build_dir = os.path.join('NEVE_run', build_dir)
